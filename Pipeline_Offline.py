...
class Pipeline_Offline:
    ...
    def NNTrain(self, n_Examples, training_dataset,n_CV,cv_dataset):
        ...
        # Setup Dataloader
        train_data = torch.utils.data.DataLoader(training_dataset,batch_size = self.N_B,shuffle = True)
        ...
        for ti in range(0, self.N_Epochs):
            ...
            y_training, train_target = next(iter(train_data))
            ...
            for t in range(0, self.ssModel.T):
                x_out_training[:,:, t] = self.model(y_training[:,:, t]).T
